...
class ProductController extends CoreController
{
    ...
    public function bestSellerProducts(Request $request)
    {
        try {
            ...
            $limit = $this->validateLimit($request->limit ?? 10);
            $language = $request->language ?? config('shop.default_language');
            $range = $this->validateRange($request->range);
            $type_id = $request->type_id ?: '';
            $category_id = $this->validateCategoryId($request->category_id ?? '0');
            $sort_by = $request->sort_by ?? 'created_at'; // New: sorting option
            $sort_order = $request->sort_order ?? 'desc'; // New: sort order
            $include_reviews = $request->include_reviews ?? false; // New: include reviews
            $include_ratings = $request->include_ratings ?? false; // New: include ratings
            
            // Validate sort parameters
            $allowed_sort_fields = ['created_at', 'name', 'price', 'orders_count', 'rating'];
            ...
            // Handle type slug to type_id conversion
            ...
            // Build cache key for better caching
            $cache_key = "best_seller_products_{$language}_{$limit}_{$category_id}_{$range}_{$sort_by}_{$sort_order}";
            
            // Check cache first (uncomment if you want caching)
            // if (Cache::has($cache_key)) {
            //     return Cache::get($cache_key);
            // }
            
            // Get manually marked best seller products with enhanced query
            $manual_best_sellers = $this->getManualBestSellers($language, $limit, $category_id, $sort_by, $sort_order);
            
            // If we have enough manual best sellers, return them
            if ($manual_best_sellers->count() >= $limit) {
                $result = $manual_best_sellers->take($limit);
                // Cache::put($cache_key, $result, now()->addMinutes(15));
                return $this->formatResponse($result, $include_reviews, $include_ratings);
            }
            
            // Get additional products based on sales with enhanced logic
            $remaining_limit = $limit - $manual_best_sellers->count();
            $sales_based_products = $this->getSalesBasedProducts(
                $language, 
                $remaining_limit, 
                $category_id, 
                $range, 
            ...
            $best_seller_products = $manual_best_sellers->concat($sales_based_products);
            
            // Apply final sorting if needed
            if ($sort_by === 'rating') {
                $best_seller_products = $best_seller_products->sortByDesc('rating')->values();
            }
            
            $result = $best_seller_products->take($limit);
            
            // Cache::put($cache_key, $result, now()->addMinutes(15));
            return $this->formatResponse($result, $include_reviews, $include_ratings);
            
        } catch (\Exception $e) {
            \Log::error('Best seller products error: ' . $e->getMessage());
            return response()->json(['error' => 'Failed to fetch best seller products'], 500);
        }
    }
    
    /**
     * Get manually marked best seller products
     */
    private function getManualBestSellers($language, $limit, $category_id, $sort_by, $sort_order)
    {
        $query = $this->repository
            ->where(['language' => $language, 'available_for_order' => true, 'is_best_seller' => true])
        ...
        if ($sort_by === 'orders_count') {
            $query = $query->withCount('orders')->orderBy('orders_count', $sort_order);
        } else {
        ...
        return $query->get()
            ...
            ->append(['default_category_detail', 'product_combination_short'])
            ->values();
    }
    
    /**
     * Get sales-based best seller products
     */
    private function getSalesBasedProducts($language, $limit, $category_id, $range, $sort_by, $sort_order)
    {
        $query = $this->repository
            ->withCount('orders')
            ->where(['language' => $language, 'available_for_order' => true])
            ->where('is_best_seller', false) // Exclude already marked best sellers
            ->where('status', 'publish');
            
        // Apply date range filter
        if ($range) {
            $query = $query->whereDate('created_at', '>', Carbon::now()->subDays($range));
        }
        ...
        // Apply sorting
        if ($sort_by === 'orders_count') {
            $query = $query->orderBy('orders_count', $sort_order);
        } else {
            $query = $query->orderBy($sort_by, $sort_order);
        }
    ...
    /**
     * Validate and format limit parameter
     */
    ...
    /**
     * Validate and format range parameter
     */
    private function validateRange($range)
    {
        if (empty($range) || $range === 'undefined') {
            return '';
        }
        $range = (int) $range;
        return max(1, min(365, $range)); // Between 1 and 365 days
    }
    ...
    /**
     * Format response with optional reviews and ratings
     */
    private function formatResponse($products, $include_reviews = false, $include_ratings = false)
    {
        $result = $products;
        
        if ($include_reviews) {
            $result = $result->append(['reviews']);
        }
        
        if ($include_ratings) {
            $result = $result->append(['rating', 'rating_count']);
        }
