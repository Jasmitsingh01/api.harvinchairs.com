...
class CategoryController extends CoreController
{
    ...
    public function index(Request $request)
    {
        ...
        $limit = $request->limit ?   $request->limit : 15;
        if ($parent === null) {
            return $this->repository->with(['type', 'parent', 'children'])->where('parent', config('constants.parent_id'))->where('language', $language)->where('enabled',true)->orderBy('position','ASC')->paginate($limit)->makeHidden(['cover_image','thumbnail_image']);
        }
    ...
    public function topCategories(Request $request)
    {
        ...
        if ($parent === null) {
            return $this->repository->with(['type', 'parent', 'children'])->where('parent', config('constants.parent_id'))->where('enabled',true)->where('language', $language)->orderBy('position','ASC')->paginate($limit)->makeHidden(['cover_image','thumbnail_image']);
        }
        elseif(is_numeric($parent)){
            return $this->repository->with(['type', 'parent', 'children'])->where('parent', $parent)->where('language', $language)->where('enabled',true)->orderBy('position','ASC')->paginate($limit)->makeHidden(['cover_image','thumbnail_image']);
        } else {
