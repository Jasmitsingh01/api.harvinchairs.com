    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.static-pages.update", [$staticPage->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $staticPage->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $staticPage->slug) }}" required>
                @if($errors->has('slug'))
                    <div class="invalid-feedback">
                        {{ $errors->first('slug') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('meta_title') ? 'is-invalid' : '' }}" type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $staticPage->meta_title) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('meta_description') ? 'is-invalid' : '' }}" name="meta_description" id="meta_description">{{ old('meta_description', $staticPage->meta_description) }}</textarea>
  ...
  for (var i = 0; i < allEditors.length; ++i) {
    ClassicEditor.create(
                    allEditors[i], {
                        ...
                        extraPlugins: [SimpleUploadAdapter, 'MediaEmbed'],
                        // Allow iframes in the content
                        htmlSupport: {
                            allow: [
                                {
                                    name: 'iframe',
