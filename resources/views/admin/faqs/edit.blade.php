        ...
        <form method="POST" action="{{ route("admin.faqs.update", [$faq->id]) }}" enctype="multipart/form-data">
            ...
            {{-- <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $faq->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('faq_type_id') ? 'is-invalid' : '' }}" name="faq_type_id" id="faq_type_id" required>
                    ...
                    @foreach($faqtypes as $id => $entry)
                        <option value="{{ $entry->id }}" {{ (old('faq_type_id') ? old('faq_type_id') : $faq->faq_type_id ?? '') == $entry->id ? 'selected' : '' }}>{{ $entry->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $faq->status || old('status', 0) === 1 ? 'checked' : '' }}>
