    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.special-offers.update", [$specialOffer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $specialOffer->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('offer_type') ? 'is-invalid' : '' }}" type="text" name="offer_type" id="offer_type" value="{{ old('offer_type', $specialOffer->offer_type) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\SpecialOffer::DISCOUNT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('discount_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="discount_type_{{ $key }}" name="discount_type" value="{{ $key }}" {{ old('discount_type', $specialOffer->discount_type) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $specialOffer->discount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order_total_condition') ? 'is-invalid' : '' }}" type="number" name="order_total_condition" id="order_total_condition" value="{{ old('order_total_condition', $specialOffer->order_total_condition) }}" step="0.01">
