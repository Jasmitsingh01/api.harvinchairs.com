    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shopping-carts.update", [$shoppingCart->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $shoppingCart->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_address') ? 'is-invalid' : '' }}" type="number" name="delivery_address" id="delivery_address" value="{{ old('delivery_address', $shoppingCart->delivery_address) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('billing_address') ? 'is-invalid' : '' }}" type="number" name="billing_address" id="billing_address" value="{{ old('billing_address', $shoppingCart->billing_address) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="total" value="{{ old('total', $shoppingCart->total) }}" step="0.01">
                @if($errors->has('total'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" type="text" name="language" id="language" value="{{ old('language', $shoppingCart->language) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_empty') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_empty" id="is_empty" value="1" {{ $shoppingCart->is_empty || old('is_empty', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_confirm') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_confirm" id="is_confirm" value="1" {{ $shoppingCart->is_confirm || old('is_confirm', 0) === 1 ? 'checked' : '' }}>
