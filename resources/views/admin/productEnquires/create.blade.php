...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.productEnquire.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-enquires.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('product_title') ? 'is-invalid' : '' }}" type="text" name="product_title" id="product_title" value="{{ old('product_title', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_attributes') ? 'is-invalid' : '' }}" name="product_attributes_id" id="product_attributes_id">
                    @foreach($product_attributes as $id => $entry)
                        <option value="{{ $id }}" {{ old('product_attributes_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
