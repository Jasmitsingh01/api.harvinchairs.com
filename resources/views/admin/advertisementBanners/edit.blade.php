...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.advertisementBanner.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.advertisement-banners.update", [$advertisementBanner->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.advertisementBanner.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $advertisementBanner->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $advertisementBanner->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $advertisementBanner->active || old('active', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" name="link" id="link">{{ old('link', $advertisementBanner->link) }}</textarea>
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
            ...
            <div class="form-group">
                <label for="link_open">{{ trans('cruds.advertisementBanner.fields.link_open') }}</label>
                {{-- <input class="form-control {{ $errors->has('link_open') ? 'is-invalid' : '' }}" type="text" name="link_open" id="link_open" value="{{ old('link_open', $advertisementBanner->link_open) }}"> --}}
                <input type="radio" name="link_open" id="link_open" value="_blank" {{ old('link_open', $advertisementBanner->link_open) == "_blank" ? 'checked' : '' }} required> Open in new tab &nbsp;&nbsp;&nbsp;&nbsp;
                <input type="radio" name="link_open" id="link_open" value="_self" {{ old('link_open', $advertisementBanner->link_open) == "_self" ? 'checked' : '' }} required> Open in same tab
