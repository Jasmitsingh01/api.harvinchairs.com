    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.attributes.update", [$attribute->id]) }}" enctype="multipart/form-data">
            ...
            {{-- <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $attribute->slug) }}">
                @if($errors->has('slug'))
            ...
            {{-- <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('language') ? 'is-invalid' : '' }}" type="text" name="language" id="language" value="{{ old('language', $attribute->language) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" type="number" name="position" id="position" value="{{ old('position', $attribute->position) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('public_name') ? 'is-invalid' : '' }}" type="text" name="public_name" id="public_name" value="{{ old('public_name', $attribute->public_name) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('group_type') ? 'is-invalid' : '' }}"  name="group_type" id="group_type" >
                    <option  value="select" {{ (old('group_type') ? old('group_type') : $attribute->group_type ?? '') == 'select' ? 'selected' : '' }}>Dropdown</option>
                    <option value="radio" {{ (old('group_type') ? old('group_type') : $attribute->group_type ?? '') == 'radio' ? 'selected' : '' }}>Radio</option>
                    <option value="image_radio" {{ (old('group_type') ? old('group_type') : $attribute->group_type ?? '') == 'image_radio' ? 'selected' : '' }}>Image Radio</option>
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shop') ? 'is-invalid' : '' }}" name="shop_id" id="shop_id">
                    @foreach($shops as $id => $entry)
                        <option value="{{ $id }}" {{ (old('shop_id') ? old('shop_id') : $attribute->shop->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('shop'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_fabric') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_fabric" id="is_fabric" value="1" {{ $attribute->is_fabric || old('is_fabric', 0) === 1 ? 'checked' : '' }}>
