...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" id="createMenu" action="{{ route("admin.menus.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group mt-4">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" value="1" name="is_cms" id="is_cms_true"  {{old('is_cms') == '1' ? 'checked' : '' }}>
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" value="0" name="is_cms"
                        id="is_cms_false"  {{ old('is_cms') == '0' ? 'checked' : '' }}>
            ...
            <div class="form-group" id="cms_div" style="display: none;">
            ...
            <div class="form-group">
                <label for="color_code">{{ trans('cruds.banner.fields.text_colour') }}</label><br>
                  <input type="text" class="coloris instance2 form-control {{ $errors->has('color_code') ? 'is-invalid' : '' }}" value="{{ old('color_code', '') }}" name="color_code" id="color_code">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('text_bold') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="text_bold" value="0">
                    <input class="form-check-input" type="checkbox" name="text_bold" id="text_bold" value="1" {{ old('text_bold', 0) == 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="text_bold">Is Bold</label>
                </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_mega_menu') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="is_mega_menu" value="0">
                    <input class="form-check-input" type="checkbox" name="is_mega_menu" id="is_mega_menu" value="1" {{ old('is_mega_menu', 0) == 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_mega_menu">Is Mega Menu</label>
                </div>
                @if($errors->has('is_mega_menu'))
                    <div class="invalid-feedback">
                        {{ $errors->first('is_mega_menu') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.menu.fields.active_helper') }}</span>
            </div>

            <div class="form-group" id="banner-field">
            ...
            <div class="form-group">
                ...
                <a class="btn btn-primary" href="{{route('admin.menus.index')}}">
                    {{ trans('global.cancel') }}
                </a>
            </div>
        </form>
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
        // Initially hide the banner field if "Is Mega Menu" is unchecked
        if (!$('#is_mega_menu').prop('checked')) {
            $('#banner-field').hide();
        }

        // Toggle the visibility of the banner field when the checkbox is clicked
        $('#is_mega_menu').change(function() {
        ...
        document.getElementById('createMenu').addEventListener('submit', function () {
            const isMegaMenu = document.getElementById('is_mega_menu');
            const banner = document.getElementById('banner');

            // If is_color checkbox is not checked, set color_code value to null
            if (!isMegaMenu.checked) {
                banner.value = null;
            }
        });
    });
    ...
    <script>
        ...
        init: function () {
    @if(isset($menu) && $menu->banner)
          var file = {!! json_encode($menu->banner) !!}
