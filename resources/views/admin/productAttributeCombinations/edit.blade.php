...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productAttributeCombination.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-attribute-combinations.update", [$productAttributeCombination->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_attribute') ? 'is-invalid' : '' }}" name="product_attribute_id" id="product_attribute_id">
                    @foreach($product_attributes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_attribute_id') ? old('product_attribute_id') : $productAttributeCombination->product_attribute->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('attribute') ? 'is-invalid' : '' }}" name="attribute_id" id="attribute_id">
                    @foreach($attributes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('attribute_id') ? old('attribute_id') : $productAttributeCombination->attribute->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('attribute_value') ? 'is-invalid' : '' }}" name="attribute_value_id" id="attribute_value_id">
                    @foreach($attribute_values as $id => $entry)
                        <option value="{{ $id }}" {{ (old('attribute_value_id') ? old('attribute_value_id') : $productAttributeCombination->attribute_value->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
